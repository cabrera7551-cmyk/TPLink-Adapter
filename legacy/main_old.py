...
def pause():
    input(f"\n{DIM}Presiona {YELLOW}ENTER{RESET}{DIM} para continuar...{RESET}")

def separator():
    line = "═" * 41
    half = len(line) // 2
    print(f"{CYAN}{line[:half]}{BLUE}{line[half:]}{RESET}")

def header(title):
    clear()
    separator()
    print(f"{CYAN}{BOLD}{title:^41}{RESET}")
    separator()
    print()
...
def change_managed_interface():
    ...
    if not interfaces:
        ...
        return
    if len(interfaces) == 1:
        info("Solo hay una interfaz Wi-Fi disponible:", interfaces[0], CYAN)
        global _SELECTED_INTERFACE
        _SELECTED_INTERFACE = interfaces[0]
        pause()
        return
...
def get_phy(interface):
    if not interface:
        return "Desconocido"
    current_phy = None
    for line in run_argv(["iw", "dev"]).splitlines():
        stripped = line.strip()
        if stripped.startswith("phy#"):
            current_phy = stripped.replace("phy#", "phy")
        elif stripped.startswith("Interface ") and stripped.split()[1] == interface and current_phy:
            return current_phy
    return "Desconocido"
...
def adapter_information():
    ...
    info("Modelo:", adapter['model'], CYAN)
    info("Chipset:", adapter['chipset'], CYAN)
    info("USB ID:", adapter['usb_id'], MAGENTA)
    info("Interfaz:", interface or 'No detectada', CYAN)
    if interface:
        info("MAC:", get_mac(interface), MAGENTA)
        info("PHY:", get_phy(interface), CYAN)
...
def supported_modes_menu():
    ...
    for mode in modes:
        print(f"{GREEN}  ✓{RESET} {YELLOW}{mode}{RESET} {DIM}—{RESET} {WHITE}{MODE_DESCRIPTIONS.get(mode, 'Modo Wi-Fi soportado por el controlador.')}{RESET}")
    pause()
...
def band_channel_menu():
    ...
    for band in bands_to_show:
        print(f"{MAGENTA}{BOLD}{band}{RESET}")
        channels = info_obj[band]["channels"]
        widths = info_obj[band]["widths"]
        print(f"  {WHITE}Canales:{RESET} {CYAN}{', '.join(map(str, channels)) if channels else f'{RED}No detectados{RESET}'}{RESET}")
...
def adapter_status():
    ...
    info("Interfaz:", interface, CYAN)
    info("Estado:", get_interface_state(interface), GREEN)
    info("Modo:", get_current_mode(interface), YELLOW)
    info("Canal:", get_current_channel(interface), MAGENTA)
    print()
...
def diagnostic():
    ...
    info("[2] Modelo:", adapter['model'], CYAN)
    info("[3] Chipset:", adapter['chipset'], CYAN)
    info("[4] Interfaz:", interface or 'No detectada', CYAN)
    if interface:
        info("[5] PHY:", get_phy(interface), CYAN)
...
def reconnect_wifi():
    ...
    info("Interfaz Wi-Fi:", interface, CYAN)
    print(
        f"{CYAN}Estado:{RESET} {WHITE}{get_interface_state(interface)}{RESET}  "
        f"{CYAN}Carrier:{RESET} {WHITE}{get_carrier(interface)}{RESET}  "
        f"{CYAN}IPv4:{RESET} {WHITE}{get_ipv4(interface) or 'Sin IP'}{RESET}"
    )
    print()
    ...
    for idx, net in enumerate(networks, start=1):
        ...
        print(f"{GREEN}[{idx}]{RESET} {lock} {WHITE}{net['ssid']:<25}{RESET} {DIM}señal: {net['signal']}%{RESET}")
    ...
    print()
    separator()
    print(f"{WHITE}{BOLD}Estado de dispositivos:{RESET}")
...
def get_connection_details(interface):
    ...
    if not link_output or "Not connected" in link_output:
        return None
...
def current_connection_info():
    ...
    info("BSSID:", details["bssid"], MAGENTA)
    info("Frecuencia:", details["frequency"], CYAN)
    info("Canal:", details["channel"], CYAN)
    ...
    print()
    separator()
    print(f"{WHITE}{BOLD}PRUEBAS{RESET}")
    separator()
...
def detailed_driver_info():
    ...
    if output: print(f"{DIM}{output}{RESET}")
    pause()
...
def verify_driver():
    ...
    if driver == "Desconocido":
        ...
        info("Esto no necesariamente es un error;", "revisa si el adaptador funciona bien igual.", DIM)
...
def repair_driver():
    ...
    print()
    info("Paquete:", adapter['package'], MAGENTA)
...
def driver_menu():
    while True:
        ...
        print(f"{CYAN}[2]{RESET} Información detallada del driver")
        print(f"{MAGENTA}[3]{RESET} Verificar compatibilidad")
        print(f"{DIM}[0] Regresar{RESET}")
        choice = input(f"\n{YELLOW}❯{RESET} {WHITE}Selecciona:{RESET} ").strip()
...
def main_menu():
    while True:
        ...
        separator()
        info("Adaptador:", interface or 'No detectado', CYAN)
        info("Modelo:", adapter['model'], CYAN)
        info("Chipset:", adapter['chipset'], CYAN)
        print()
        print(f"{GREEN}[1]{RESET} Información del adaptador")
        print(f"{CYAN}[2]{RESET} Modos soportados")
        print(f"{BLUE}[3]{RESET} Bandas y canales")
        print(f"{YELLOW}[4]{RESET} Estado del adaptador")
        print(f"{MAGENTA}[5]{RESET} Diagnóstico")
        print(f"{RED}[6]{RESET} Cambiar modo")
        print(f"{GREEN}[7]{RESET} Driver")
        print(f"{BLUE}[8]{RESET} Estado de red (todas las interfaces)")
        print(f"{MAGENTA}[9]{RESET} Reparar conexión a Internet (Wi-Fi)")
        print(f"{CYAN}[10]{RESET} Información de conexión actual")
        print(f"{WHITE}[11]{RESET} Cambiar interfaz Wi-Fi administrada")
        print()
        print(f"{DIM}[0] Salir{RESET}")
